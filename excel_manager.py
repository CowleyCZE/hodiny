...
class ExcelManager:
    ...
    def _get_cell_coordinates(self, field_key, sheet_name=None):
        """Vrátí (row, col) souřadnice pro daný field z dynamické konfigurace.
        ...
        Returns:
            tuple: (row, col) nebo None pokud není nakonfigurováno
        """
        config = self._load_dynamic_config()
        field_config = config.get(field_key)
        ...
        if field_config.get('file') != self.active_filename:
            logger.warning("Konfigurace pro %s odkazuje na jiný soubor: %s", 
                         field_key, field_config.get('file'))
            return None
            
        if sheet_name and field_config.get('sheet') != sheet_name:
            logger.warning("Konfigurace pro %s odkazuje na jiný list: %s (očekáván %s)", 
        ...
        try:
            return coordinate_to_tuple(cell)  # Převede např. "A1" na (1, 1)
        except ValueError as e:
            logger.error("Neplatná buňka v konfiguraci pro %s: %s - %s", 
                        field_key, cell, e)
            return None
    ...
    def _zapsat_data_do_listu(
        ...
        for employee in employees:
            ...
            if not row_idx:
                ...
                employee_name_coords = self._get_cell_coordinates('employee_name', sheet_name)
                if employee_name_coords:
                    emp_row, emp_col = employee_name_coords
        ...
        start_time_coords = self._get_cell_coordinates('start_time', sheet_name)
        if start_time_coords:
            start_row, start_col = start_time_coords
        ...
        end_time_coords = self._get_cell_coordinates('end_time', sheet_name)
        if end_time_coords:
            end_row, end_col = end_time_coords
        ...
        lunch_coords = self._get_cell_coordinates('lunch_duration', sheet_name)
        if lunch_coords:
            lunch_row, lunch_col = lunch_coords
        ...
        total_hours_coords = self._get_cell_coordinates('total_hours', sheet_name)
        if total_hours_coords:
            total_row, total_col = total_hours_coords
        ...
        date_coords = self._get_cell_coordinates('date', sheet_name)
        if date_coords:
            date_row, date_col = date_coords
