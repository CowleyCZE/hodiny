...
class Hodiny2025Manager:
    ...
    def zapis_pracovni_doby(self, date: str, start_time: str, end_time: str, lunch_duration: str, num_employees: int):
        ...
        try:
            ...
            logger.info(
                f"Pracovní doba pro {date} byla zapsána do listu {sheet.title}, "
    ...
    def get_monthly_summary(self, month: int, year: int = 2025) -> dict:
        ...
        """
        try:
            # Load the workbook with calculated values first
            try:
                workbook_values = load_workbook(self.file_path, data_only=True)
                sheet_name = f"{month:02d}hod{str(year)[2:]}"

                if sheet_name not in workbook_values.sheetnames:
                    # If sheet doesn't exist, create it and return zeros
                    self.get_or_create_month_sheet(month, year)
                    workbook_values = load_workbook(self.file_path, data_only=True)

                sheet_values = workbook_values[sheet_name]

                # Try to get values from formulas first
                total_hours = sheet_values.cell(row=self.SUMMARY_ROW, column=self.COL_TOTAL_HOURS).value
                total_overtime = sheet_values.cell(row=self.SUMMARY_ROW, column=self.COL_OVERTIME).value
                total_all_employees = sheet_values.cell(row=self.SUMMARY_ROW, column=self.COL_TOTAL_ALL).value

            except (FileNotFoundError, InvalidFileException):
                total_hours = total_overtime = total_all_employees = None

            # If formulas returned None (not calculated), calculate manually
            if total_hours is None or total_overtime is None or total_all_employees is None:
                logger.info(f"Formule pro souhrn {month}/{year} nebyly vypočítány, počítám ručně")

                # Manual calculation by summing individual day records
                calculated_total_hours = 0.0
                calculated_total_overtime = 0.0
                calculated_total_all = 0.0

                # Get days in month and sum up the records
                days_in_month = calendar.monthrange(year, month)[1]

                for day in range(1, days_in_month + 1):
                    try:
                        date_str = f"{year}-{month:02d}-{day:02d}"
                        daily_record = self.get_daily_record(date_str)

                        if "error" not in daily_record:
                            calculated_total_hours += daily_record.get("total_hours", 0.0)
                            calculated_total_overtime += daily_record.get("overtime", 0.0)
                            calculated_total_all += daily_record.get("total_all_employees", 0.0)

                    except Exception:
                        continue  # Skip days that don't exist or have errors

                # Use calculated values if formulas failed
                if total_hours is None:
                    total_hours = calculated_total_hours
                if total_overtime is None:
                    total_overtime = calculated_total_overtime
                if total_all_employees is None:
                    total_all_employees = calculated_total_all

            return {
                "month": month,
                "year": year,
                "month_name": self.CZECH_MONTHS[month],
                "total_hours": float(total_hours) if isinstance(total_hours, (int, float)) else 0.0,
                "total_overtime": float(total_overtime) if isinstance(total_overtime, (int, float)) else 0.0,
                "total_all_employees": (
                    float(total_all_employees) if isinstance(total_all_employees, (int, float)) else 0.0
                ),
                "sheet_name": f"{month:02d}hod{str(year)[2:]}",
            }
