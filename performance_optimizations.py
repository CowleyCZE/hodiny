...
def get_excel_file_info():
    ...
    try:
        return {
            ...
            'filename': g.excel_manager.get_active_filename() if hasattr(g.excel_manager, 'get_active_filename') else 'Unknown',
...
def cleanup_old_data():
    ...
    try:
        ...
        perf_monitor.request_times = [
            r for r in perf_monitor.request_times 
            if r['timestamp'] > cutoff_time
        ]
        perf_monitor.slow_requests = [
            r for r in perf_monitor.slow_requests 
            if r['timestamp'] > cutoff_time
        ]
