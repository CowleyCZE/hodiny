...
def test_hodiny2025_manager():
    ...
    for i, record in enumerate(test_records, 1):
        try:
            manager.zapis_pracovni_doby(
                record["date"], record["start"], record["end"], record["lunch"], record["employees"]
            )
    ...
    for i, record in enumerate(test_records, 1):
        try:
            ...
            if abs(daily_data.get("total_hours", 0) - record["expected_hours"]) < 0.1:
                ...
                print(
                    f"   ❌ Chyba v celkových hodinách: očekáváno {record['expected_hours']}h, "
            ...
            if abs(daily_data.get("overtime", 0) - record["expected_overtime"]) < 0.1:
                ...
                print(
                    f"   ❌ Chyba v přesčasech: očekáváno {record['expected_overtime']}h, "
    ...
    try:
        ...
        print(
            "   Poznámka: Hodnoty mohou být 0, pokud Excel neprovedl přepočet vzorců. "
            "Denní hodnoty jsou počítány v Pythonu."
        )
